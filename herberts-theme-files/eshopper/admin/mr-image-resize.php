<?php
function eshopper_post_thumb( $width=300, $height='', $html=true) {		
  global $post;
  if( !has_post_thumbnail() ) return false;

  $fullsize = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
  $url = eshopper_image_resize( $fullsize[0], $width, $height, true, 'c', false );

  if($html){
    echo '<img src="'.esc_url($url).'" alt="'.get_the_title(get_post_thumbnail_id( $post->ID )).'">';
  }else{
    return '<img src="'.esc_url($url).'" alt="'.get_the_title(get_post_thumbnail_id( $post->ID )).'">';
  }
}
...
function eshopper_image_resize($url, $width=null, $height=null, $crop=true, $align='c', $retina=false) {
  $retina = false;
  global $wpdb;
