...
?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		...
		<header class="entry-header">
			...
			<?php
				$url = get_post_meta( get_the_ID(), 'eshopper_oembed_audio', true );
				if( $url != '' ){
					global $wp_embed;
					$post_embed = $wp_embed->run_shortcode('[embed height="150"]'.$url.'[/embed]');
					echo '<div class="audio-wrap post-embed">'.$post_embed.'</div>';
				}
			?>
