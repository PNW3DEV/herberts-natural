 ...
 * @since 2.8.0
 */
class Eshopper_Widget_Recent_Posts extends WP_Widget {

    function __construct() {
        $widget_ops = array('classname' => 'widget_recent_entries', 'description' => __( "The most recent posts on your site", THEMENAME) );
        parent::__construct('recent-posts', __('Eshopper Recent Posts', THEMENAME), $widget_ops);
...
?>
        ...
        <ul class="eshopper-recent-posts">
        <?php while ( $r->have_posts() ) : $r->the_post(); ?>
            <li>
                <?php
                    if( has_post_thumbnail() ){
                        echo '<div class="thumb-wrap pull-left">';
                        eshopper_post_thumb( 50, 50 );
                        echo '</div>';
                    }
                ?>
...
function eshopper_register_custom_widgets() {
    register_widget( 'Eshopper_Widget_Recent_Posts' );
}
add_action( 'widgets_init', 'eshopper_register_custom_widgets' );
?>